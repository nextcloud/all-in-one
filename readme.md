...
After you are done modifying/adding/deleting files/folders, don't forget to apply the correct permissions by running: `sudo docker exec nextcloud-aio-nextcloud sh -c "chown -R 33:0 /mnt/ncdata/*"` and `sudo docker exec nextcloud-aio-nextcloud sh -c "chmod -R 750 /mnt/ncdata/*"` and rescan the files with `sudo docker exec --user www-data -it nextcloud-aio-nextcloud php occ files:scan --all`.
