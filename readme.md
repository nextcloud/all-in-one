...
### How to stop borg compacting automatically?
By default, AIO will automatically call [`borg compact`](https://borgbackup.readthedocs.io/en/stable/usage/compact.html) each time it backs up. This will save repository space, so most users should leave this on. However, it may make the repository less fault-tolerant, and if you are backing up to a synced repository elsewhere, you might want to control when your borg repository is compacted - particularly when syncing to a storage service that has a minimum retention period for billing purposes. To take control of this yourself, you can provide `--env BORG_SKIP_COMPACTING=true` to the mastercontainer in the same way described just above to change retention options (https://github.com/nextcloud/all-in-one#how-to-enable-automatic-updates-without-creating-a-backup-beforehand). Note that enabling this means that repository space will not be freed unless and until you run `borg compact` manually. For more information, see [the Borg docs on separate compaction](https://borgbackup.readthedocs.io/en/stable/usage/notes.html#separate-compaction).
