...
If you want to use a reverse-proxy on another host, so not running on the same docker-daemon as Nextcloud-Aio, but on a second host, you can see a working example configuration described [here](https://github.com/hanscees/nextcloud-aio-remote-reverse-proxy), using nginx. 
