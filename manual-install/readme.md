...
- For updating, you need to strictly follow the update routine described below
- Probably more
...
```
Then copy the sample.conf file to default environment file, e.g. `cp sample.conf .env`, open the new conf file, e.g. with `nano .env`, edit all values that are marked with `# TODO!`, close and save the file.
⚠️ **Warning**: Do not use the symbols `@` and `:` in your passwords. These symbols are used to build database connection strings. You will experience issues when using these symbols! Also, please note that changes to values inside the latest.yaml that are not exposed as variables are not officially supported. See for example [this report](https://github.com/nextcloud/all-in-one/issues/5612).

Then copy the provided yaml file to a compose.yaml file by running `cp latest.yml compose.yaml`.

Now you should be ready to go with `sudo docker compose up`.

## Docker profiles
The default profile of `latest.yml` only runs the minimum necessary services: nextcloud, database, redis and apache. To start optional services collabora, talk, whiteboard, talk-recording, clamav, imaginary or fulltextsearch use additional arguments for each of them, for example `--profile collabora`.

For a complete all-in-one-with-collabora platform use `sudo docker compose --profile collabora --profile talk --profile talk-recording --profile clamav --profile imaginary --profile fulltextsearch --profile whiteboard up`.

## How to update?
Since the AIO containers may change in the future, it is highly recommended to strictly follow the following procedure whenever you want to upgrade your containers.
1. Run `sudo docker compose down` to stop all running containers
2. Back up all important files and folders
3. If your previous copy of `sample.conf` is named `my.conf`, run `mv -vn my.conf .env` in order to rename the file to `.env`.
...
7. After the file updates were successful, simply run `sudo docker compose pull` to pull the new images.
