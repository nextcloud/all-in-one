...
OUTPUT="$(echo "$OUTPUT" | jq '.services[] |= if has("depends_on") and .depends_on.contains(["nextcloud-aio-docker-socket-proxy"]) then del(.depends_on.[index("nextcloud-aio-docker-socket-proxy")]) else . end')"
