...
# shellcheck disable=SC1083
find ./ -name '*apache*' -exec sed -i "s|$APACHE_PORT|{{ .Values.APACHE_PORT }}|; /^spec:/a\ \ type: LoadBalancer\n" \{} \;
# shellcheck disable=SC1083
find ./ -name '*talk*' -exec sed -i "s|$TALK_PORT|{{ .Values.TALK_PORT }}|; /^spec:/a\ \ type: LoadBalancer\n" \{} \;
# shellcheck disable=SC1083
