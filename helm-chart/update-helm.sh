...
# shellcheck disable=SC1083
find ./ -name '*elasticsearch-persistentvolumeclaim.yaml' -exec sed -i "s|storage: 1Gi|storage: {{ .Values.ELASTICSEARCH_STORAGE_SIZE }}|" \{} \;  
# shellcheck disable=SC1083
...
# shellcheck disable=SC2129
echo 'ELASTICSEARCH_STORAGE_SIZE: 1Gi       # You can change the size of the elasticsearch volume with this value' >> /tmp/sample.conf
