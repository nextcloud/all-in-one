 ...
 * Copied over from https://github.com/nextcloud/server/blob/master/build/license.php
 */
...
EOD;
	...
	public function exec($folder, $gitRoot = false) {
		...
		$iterator = new RegexIterator($iterator, '/^.+\.(js|php|md|twig|xml|Dockerfile|Caddyfile|sh|conf|script|cfg|motd|yml|yaml|css)$/i');
...
';
	...
	public function handleFile($path, $gitRoot) {
		...
		if ($isPhp) {
            ...
            $source = '#!/bin/bash' . PHP_EOL . $license . PHP_EOL . PHP_EOL . $source;
        } else {
