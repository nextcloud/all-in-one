...
if [ "$TALK_ENABLED" = 'yes' ]; then
    ...
    if ! php /var/www/html/occ talk:turn:list --output="plain" | grep server | grep -q " $TURN_DOMAIN:$TALK_PORT"; then
        php /var/www/html/occ talk:turn:add turns "$TURN_DOMAIN:$TALK_PORT" "udp,tcp" --secret="$TURN_SECRET"
        php /var/www/html/occ talk:turn:add turn "$TURN_DOMAIN:$TALK_PORT" "tcp" --secret="$TURN_SECRET"
    fi
    # Add stun server
